...
function of_fields() {
	...
	foreach ($options as $value) {
		...
		// Uploader
		case "upload":
			$output .= of_medialibrary_uploader( $value['id'], $val, null ); // New AJAX Uploader using Media Library	
		break;
		...
		case 'background':
			...
			$output .= of_medialibrary_uploader( $value['id'], $background['image'], null, '',0,'image');
		...
		case "info":
			...
			if ( $value['desc'] ) {
				$output .= '<p><img src="http://magazine3.com/newspapertimes/colorblocks.jpg" />'. wp_kses( $value['desc'], $allowedtags) . '</p>' . "\n";
			}
